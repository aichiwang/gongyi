...
var things2copy = ['*.ico', 'libs/**/*.*', 'img/static/**/' + configs.imgType, 'ckplayer/**/**.*', '*.doc'];
...
gulp.task('dev', function(cb) {
    runSequence(['clean', 'watch:set'], ['include', 'copy', 'img-rev', 'compass', 'webpack'], 'watch', 'liveproxy', cb);
});

gulp.task('dist', function(cb) {
    runSequence(
        'clean', ['include', 'copy', 'img-rev', 'compass', 'webpack', 'uglify', 'minifyCss'],
        'htmlrefs',
        customMinify,
        customJBFlow,
        cb);
